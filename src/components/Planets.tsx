...
export const RealisticPlanet = ({
  ...
  return (
    ...
    >
      ...
      {/* Atmospheric halo OUTSIDE the planet body — soft glow that hides
          any aliasing on the planet's rounded edge and adds realism */}
      <div
        className="absolute inset-[-10%] rounded-full pointer-events-none z-[5]"
        style={{
          background:
            'radial-gradient(circle, transparent 48%, rgba(255,255,255,0.12) 56%, rgba(255,255,255,0.04) 64%, transparent 78%)',
          filter: 'blur(2px)',
        }}
      />

      {/* Planet Body */}
      <div className={`relative w-full h-full rounded-full overflow-hidden ${planet.baseBg} ${planet.id === 'uranus' ? 'blur-[1px]' : ''} z-10`}>
        ...
        {/* Realistic Moon/Planet Surface Noise (softened) */}
        <div className={`absolute inset-0 opacity-30 mix-blend-soft-light ${planet.id === 'earth' ? 'opacity-70 mix-blend-soft-light' : ''}`} style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.85' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E")` }}></div>
        <div className="absolute inset-0 opacity-15 mix-blend-multiply" style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter2'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.15' numOctaves='2' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter2)'/%3E%3C/svg%3E")` }}></div>
        ...
        {/* Sunlight hot spot — bright specular highlight from upper-left */}
        {planet.id === 'venus' ? (
           <div className="absolute inset-0 rounded-full bg-[radial-gradient(circle_at_18%_28%,rgba(255,255,255,0.55)_0%,transparent_38%)] mix-blend-screen pointer-events-none" />
        ) : planet.id === 'mercury' ? (
           <div className="absolute inset-0 rounded-full bg-[radial-gradient(circle_at_15%_25%,rgba(255,255,255,0.45)_0%,transparent_42%)] mix-blend-screen pointer-events-none" />
        ) : (
           <div className="absolute inset-0 rounded-full bg-[radial-gradient(circle_at_25%_25%,rgba(255,255,255,0.45)_0%,transparent_45%)] mix-blend-screen pointer-events-none" />
        )}

        {/* Terminator + night side — smooth gradient from lit side to dark
            side, gives a real 3D illuminated-sphere feel without the
            harsh inset boxes that previously ate the edge */}
        <div
          className="absolute inset-0 rounded-full pointer-events-none"
          style={{
            background:
              'radial-gradient(circle at 28% 28%, rgba(0,0,0,0) 35%, rgba(0,0,0,0.45) 65%, rgba(0,0,0,0.85) 95%)',
          }}
        />

        {/* Limb darkening — subtle dark ring all the way around so edges
            look round and 3D (real planets are darker near the limb).
            Also smooths out any single-pixel rasterization artifacts on
            the rounded mask. */}
        <div
          className="absolute inset-0 rounded-full pointer-events-none"
          style={{
            background:
              'radial-gradient(circle, transparent 55%, rgba(0,0,0,0.30) 88%, rgba(0,0,0,0.55) 100%)',
